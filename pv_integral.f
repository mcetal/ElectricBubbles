...
C********x*********x*********x*********x*********x*********x*********x**
C
      SUBROUTINE PVINTEV(K0,K,ND,NMAX,X,Y,X2,Y2,h,UCMPLX,
     *                   U2,QA,CFIELD,NSP,WKSP,PHICMPLX)
c
c     THIS SUBROUTINE DOES NOT ALLOW DIFFERENT POINTS ON EACH HOLE.
C
...
C          etc.
C
C     ND     = number of points on each hole
...
C
      ...
      INTEGER *4 IOUT(2),INFORM(10),IERR(10), nd(k)
      COMPLEX*16 UCMPLX(NMAX),PHICMPLX(NMAX),EYE,ZERO, z2pii,
     *           QA(2*NMAX),U2(2*NMAX),CFIELD(2*NMAX),WKSP(NSP)
      DIMENSION X(NMAX),Y(NMAX),X2(2*NMAX), h(k0:k),
     *          Y2(2*NMAX),POTEN(1)
...
C
     ...
     1                IP2,IPT)
         ...
         CALL FINTER(x(ISTART+1),x2(istart2+1),NDM1,ND2M1,WKSP,NSP,
     1                IP2,IPT)
         CALL FINTER(y(ISTART+1),y2(istart2+1),NDM1,ND2M1,WKSP,NSP,
     1                IP2,IPT)
         DO I = 2,ND2,2
ccc            QA(ISTART2+I) = 4.d0*PI*U2(I)/ND2
            QA(ISTART2+I) = U2(I)*h(nbod)
         ENDDO
...
C
      ...
      DO NBOD = K0,K
         DO I = 1,nd(nbod)
            I2 = 2*I-1
            PHICMPLX(ISTART+I) = -z2pii*CFIELD(ISTART2+I2)
         ENDDO
         ISTART = ISTART+nd(nbod)
...
140   CONTINUE
      do 160 i = NCORSE+3,2*NFINE-NCORSE+2
	 WORK(IPT+I) = 0
160   CONTINUE
