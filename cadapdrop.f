C
C**********************************************************************C
      SUBROUTINE CADAPDR(IFLAG1,IFLAG2,LEVMAX,XAT,YAT,DZDT,K0,KBOD,ND,
     1           DENS,H,IWORK,LENIW,CWORK,LENW,XTARG,YTARG,NTARG,
     2           GRADW,PHIP,PHIPP,TOL,INFORM,IER,XLEFT,SLENGTH)
C**********************************************************************C
C
c  FOR THE DROP INTERFACE PROBLEM, WHERE 
c     GRADW = PHI - Z conjg(PHI') - conjg(PSI)
c  wall bounded problem has not been debugged
c  
C     INPUT:
C
...
C
	 IF (IFLAG1.EQ.0) THEN
            CALL DOINT0DR(IFLAG2,ILEV,CWORK(IZSRC),CWORK(IQA),
     ...
     6        NTBOX,GRADW(1))
	 ELSE
            CALL DOINT1DR(IFLAG2,ILEV,CWORK(IZSRC),CWORK(IQA),
     1        CWORK(IQB),CWORK(IQC),CWORK(IZSRC2),CWORK(IQA2),
     2        CWORK(IQB2),CWORK(IQC2),NATOMS,CWORK(IZTARG),NTARG,
     3        CWORK(ILOCXP1),CWORK(ILOCXP2),NTERMS,IWORK(IOFFST),
     4        IWORK(IBOX),IWORK(IATADR),IWORK(ITRADR),IWORK(ITOFST),
     ...
     6        NTBOX,GRADW(1),PHIP(1),PHIPP(1))
         ENDIF
         call prin2(' after DOINT0 GRADW = *',GRADW,2*NATOMS)
...
1000  CONTINUE
1001  CONTINUE
ccc      call prin2(' after all far fields GRADW = *',GRADW,2*NATOMS)
      call prinf(' doing near neighbors *',ILEV,0)
      call prinf(' ILEV =  *',ILEV,1)
ccc      timen = second()
      IF (IFLAG1.EQ.0) THEN
         CALL DONN0DR(IFLAG2,ILEV,CWORK(IZSRC),CWORK(IQA),CWORK(IQB),
     1      CWORK(IQC),NATOMS,CWORK(IZSRC2),CWORK(IQA2),CWORK(IQB2),
     2      CWORK(IQC2),IWORK(IOFFST),IWORK(IBOX),IWORK(IATADR),
     3      GRADW(1))
      ELSE
         CALL DONN1DR(IFLAG2,ILEV,CWORK(IZSRC),CWORK(IQA),CWORK(IQB),
     1      CWORK(IQC),NATOMS,CWORK(IZSRC2),CWORK(IQA2),CWORK(IQB2),
     ...
     4      GRADW(1),PHIP(1),PHIPP(1)) 
      ENDIF
ccc      timen2 = second()
      call prin2(' after DONN GRADW = *',GRADW,2*NATOMS)
