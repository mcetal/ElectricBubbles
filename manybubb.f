...
c
      parameter (ndmax=1024, kmax = 150, nmax = ndmax*kmax)
c
...
c
     ...
     *                 tfinal, iplot, ipost, nplot, cap, zbond, iwall)
c
...
ccc               call RSCPLOT (z(1,kbod), nd(kbod), 1, iplot)
               call RSZ_MOVIE (z(1,kbod),0.d0, nd(kbod), kbod, k,  
     1                         ntime, iplot)
...
c
            ...
            istart2 = 0
            areatot = 0.d0
            if ((time.le.tplot).and.(time+dt.gt.tplot)) 
     *               write(iplot,*) 'time = ',time
            akapmax = 0.d0
            errsmax = 0.d0
            do kbod = 1, k
               n = nd(kbod)/2
...
ccc                     call RSCPLOT (z(1,kbod), nd(kbod), 1, iplot)
                     call RSZ_MOVIE (z(1,kbod),time, nd(kbod), kbod, k,  
     1                               ntime, iplot)
                  end if
...
ccc         call PRIn2 ('  Un 2 = *', Un(1+nd(1)), nd(1))
ccc         stop
c
