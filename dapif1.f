...
C
     ...
     *                    TOL,EPS7,XA,YA,QA,POTEN,FIELD,WKSP,NSP,CLOS)
C
C**********************************************************************
C
C   *** INFORMATION:
...
CCCC    CALL PRINF('WKSP(NCHI) AFTER DAASSI IS*',WKSP(NCHI),NBOX)
	IF (IERR(1).NE.0) THEN
	   CALL DAEOUT(IERR)
...
CCCC    CALL PRINF('WKSP(NCHI) AFTER COMPRESSION*',WKSP(NCHI),NBOX)
C
